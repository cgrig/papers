...
means that the code for the design~$d_2$ is looked up, and a subsitution is
...
that the newily created agent should execute, and it is a set of rules.
...
\frac
  ...
  {\on{S_1({\it sid})=\bot}
      {S_2({\it sid})=\bigl(\{\,(a,\bot)\mid a\in{\it args}(n)\,\},\; {\it body}(n)\bigr)}}}}
...
applies. The code~$Q$ is the whole body of the session named~$n$.
...
the implementation of an action. However, there is nothing stoping a
developer from using Java reflection to access the internals of the AF-Raf
inetrpreter.  We define a \emph{well-behaved} action as one that ensures
\begin{align}
...
\todo{Perhaps reuse this when discussing action invocations.}
However, action invocations
are arbitrary Java code, which could use Java reflection to access the
internals of the interpreter and perform completely arbitrary actions. They
are effectively an escape hatch from AF-Raf's semantics, in addition to a
mean of interacting with arbitrary environments.
...
\textit{dec}(x, y) and \textit{inc}(x, y) are used in performing the task,
while action \textit{mul}(x, y) is used in calculating the bid offer. The
...
the belief base. First, it adds a belief identifying the task and the
session to the belief base, then returns the result of computing the bid
offer using the internal action \textit{mul}().
