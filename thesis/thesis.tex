...
Belief, Desire, Intention agent architecture~\cite{}, based on the human
practical reasoning theory developed by Bratman in the mid-1980s, provides
a mechanism to separate the plans selection from the current plans
execution.  The reactive planning~\cite{} represents a group of techniques
...
AgentSpeak~\cite{},
based on speech acts, MetateM~\cite{}, based on temporal logic,
3APL~\cite{}, based on
practical reasoning rules, Jason~\cite{}, which extends AgentSpeak with
communication, and GOAL~\cite{}, based on declarative goals.
...
elements required and provided by the module.
 ...
 In Haskell, modules are used to control name-spaces and to create
 abstract data types. A Haskell program can be seen as a collection of
 modules, where the main module loads up the other modules. Loading a
 module can be regarded as a big declaration. A module contains functions,
...
directly from the reference implementation of AF-Raf. The notation
\begin{align}
\texttt{AgentRole = String name, Type role;}
\end{align}
means that the AF-Raf parser may instantiate a class named {\it AgentRole},
which has two fields, {\it name\/} and {\it role}.  The abstract grammar is
a concise description of several Java classes, each having constructors,
getter methods, and so on.
...
$r_1$,~$r_2$, \dots; the rules of the design may refer to these
...
analogous to the formal arguments of a Java constructor.)
...
ExprCalc\langle value\rangle$ refer to roles, which are types for agents.
The syntax for referring to types of values is very similar: for example,
$\it List\langle int\rangle$ and {\it int}. (The type {\it int\/} is
built-in.) Type variables, like {\it value} above, always refer to types of
values, never to types of agents.
\end{example}
...
\autoref{fig:grammar-top-level} defines the grammar for the top-level
elements of an AF-Raf script.
...
declaration: java_binding | agent | role | session
           | type_declaration;
...
symbol.  In turn, a directive is either an include, a java binding, or a
...
A qualified$\_$id is represented by one or more ids, either LID or
UID, delimited by dots.
...
Protocol\cite{DBLP:journals/tc/Smith80a} developed by Smith and Davis in 1980, to which it adds
rejection and confirmation communicative acts. One agent, the initiator,
takes the manager role, the other agents are the participants. The manager
has a task that wants to be performed by the participants in the most
effective way possible. The effectiveness is considered with regard to some
specified properties like the price, the time to completion, or the fair
distribution of tasks. The participants have to respond before a given
deadline with either a refuse or a proposal. Negotiation continues with the
participants that proposed, from which one, several or no agent is chosen
to actually perform the task. The chosen agents have to inform about the
execution of the task indicating either the success, the failure or by
giving a result.

This case study discusses the Af-Raf implementation of a slightly modified
FIPA Contract Net Interaction Protocol. Figure~\ref{fig:contract-roles}
...
type request = request(task(), conditions, deadline());
type bid = bid(conditions);
type decision = win() | lose();
type result = failure() | success(integer);
  // success could have, say, price after adding VAT and other surprises

role Buyer {
  ask : request -> bid;
  commit : decision -> solution;
}
