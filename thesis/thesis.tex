...
First hypothesis states that the introduction of role as a first class
...
\begin{itemize}
  \item \textbf{Role}, is the construct that brings together related behaviour, 
     it gives a general description, but does not constraint the specific
     implementation.
...
\begin{itemize}
  \item "\textbf{autonomy}: agents operate without the direct intervention
      ...
      and internal state (Castelfranchi, 1995);"~\cite{DBLP:journals/ker/WooldridgeJ95}
    \item "\textbf{social ability}: agents interact with other agents (and
      ...
      (Genesereth and Ketchpel, 1994);"~\cite{DBLP:journals/ker/WooldridgeJ95}
    \item "\textbf{reactivity}: agents perceive their environment, (which may
      ...
      combined), and respond in a timely fashion to changes that occur in it;"~\cite{DBLP:journals/ker/WooldridgeJ95}
...
conversation scenarios of multiparty sessions.~\cite{}
...
\todo{figure with global type grammar from Nabuco's paper, add citation as
well}
...
\end{tabular}
\end{center}
\caption{Syntax of Global Types~\cite{dblp:conf/popl/hondayc08}}
\label{fig:globalType}
\end{figure} % >>>

The grammar of global session type, or global type, denoted by $G$,$G'$,...
is given in \ref{fig:globalType}. Type $\l p->p':k<U>G$says that
participant $p$ sends a message of type $U$ to channel $k$, is received by
participant $p'$, and interactions described in $G$ takes place. Type $
p->p:k{lj : Gj}j∈J$ says participant $p$ sends one of the labels to channel
$k$ which is then received by participant $p$. If $lj$ is sent,
interactions described in $Gj$ take place.  Type $G,G'$ represents
concurrent run of interactions specified by $G$ and $G'$ . Type $μt.G$ is a
recursive type for recurring conversation structures.
...
processes. The grammar is given in~\ref{fig:localType}.

\todo{cite Nabuco's paper}

The following rules define how the projection is performed from the global
type to the local types of each participant. The value global type is
...
the behaviour of waiting for an option to be selected) local type.

Nabuco~\cite{} describes the Programming Methodology for Multiparty
Interactions as follows: "Once given global types as our tool, we can
consider the following development steps for programs with multiparty
sessions.  Step 1 A programmer describes an intended interaction scenario
as global type G, and checks that it is linear.  Step 2 She develops code,
one for each participant, incrementally validating its conformance to the
projection of G onto each participant by efficient type-checking.  When
programs are executed, their interactions are guaranteed to follow the
stipulated scenario. The type specification also serves as a basis for
...
Each of the matchings from the set of query results says with what term
...
% >>>
% >>>
\chapter{Readability and Usability Survey} % <<<

This chapter investigates further our research question:
\textit{Is the readability and usability of an agent oriented programming
  language improved by the addition of the role and session programming
  constructs as well as by the algebraic data types and the type checking
functionality?} I have designed a survey (see Annex) to compare the
...
related to the Jason implementation, and type B presents questions related
to the AF-Raf implementation. The structure of the questionnaires is as
follows:
...
questions. There were 10 students (all males) who were assigned the
questionnaire type A and 10 students (1 female, 9 males) who were assigned
...
correlated to the scale used for evaluation the answers from the
