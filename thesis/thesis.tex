...
Following the research presented in this thesis Alessandro Ricci  and
Andrea Santi developed a new programming language, simpAL, to support types
and type checking~\cite{}. SimpAl draws inspiration from the Agent and Artifacts model developed by
Andrea Omicini and from the Jason programming language, which is based on
...
Even though their language is very different from AF-Raf, there are a
number of overlapping concepts. They import a social view of computation
...
The roles are organised in a slightly different way from the AF-Raf's
session type model, which can be seen as a kind of protocol prescribing the
agent communication. In simpAL organisation models are used to define
organisation types. Organisation types are implemented by concrete
organisations that bring together related roles and artifact in coherent
units named workspaces.

A number of static type checks are performed. At the role level
Guido Boella on Roles, Graversen on Roles in OOP,

D Ancona on session types,
