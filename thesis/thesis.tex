...
%\omit\hidewidth \bf{simpAL}\hidewidth&&
...
Ricci  and Santi developed a novel programming language, called simpAL,
that supports organisational structure, types and type
...
\todo{Explain how operations are desugared into rules.}
...
\item the state of the call, which is one of `not started', `call placed',
...
(undefined). The expression $[a\mapsto b]$ denotes the singleton partial
map $\{(a,b)\}$. The concatenation $\sigma_1\sigma_2$ denotes the update
...
grammar (\autoref{sec:langdef.syntax}).
...
assignments, {\bf new} statements, action invocations, and belief updates.
\todo{First introduce the notations used below.}

\begin{align}
  \frac
  {\langle\Gamma_1,T_1,C\rangle\to\langle\Gamma_2,T_2,t_2\rangle}
  {\langle\Gamma_1,T_1,x:=C\rangle\to\langle\Gamma_2[x\mapsto t_2],T_2,t_2\rangle}
\\
  \frac
  {?}
  {\langle\Gamma,T,{\bf new}\,{\bf session}\rangle\to\langle\Gamma,T,n\rangle}
\\
  \frac
  {?}
  {\langle\Gamma,T,{\bf new}\,{\bf call}\rangle\to\langle\Gamma,T,n\rangle}
\\
  \frac
  {?}
  {\langle\Gamma,T,{\bf new}\,d(a_1,\ldots,a_k)\,n\,\{T'\}\rangle\to\langle\Gamma,T,n\rangle}
\end{align}

\todo{What is below is quite old. Remove after you make sure that all the
information was already incorporated in the main text.}
...
participant). This particular example uses three agents, one seller and
two buyers.
...
First, a number of supportive types are defined: conditions, task, request,
bid, decision, result.

The role Buyer defines two operations ask and commit. The operation ask
states that upon receiving a message of type request the agent must reply
with a message of type bid. The operation commit states that upon receiving
a message of type decision the agent has to reply with a message of type
result.

The role seller defines only one operation, the operation sell, which
states that upon receiving a message of type request should reply with a
message of type result.
...
the owner which sends to the seller a message of type request. The seller
advance the request message further to each of the two buyer agents. For
each of the buyers are defined two possible scenarios: (1) The buyer sends
a message of type bid to the seller, then the seller sends a win() message,
finally, the buyer sends a message of type result to the seller; (2) The
buyer sends a message of type bit to the seller, then the seller sends a
lose() message to the buyer. The last message exchanged in the BidRound
session is a result message sent by the seller to the owner.
...
agent ContractNetSeller(b1 : BuyerRole, b2 : BuyerRole): Seller {
  operation sell(r) {
    return if b1.ask(r) < b2.ask(r) then { b1.commit(win()); b2.commit(lose());}                   
           else { b2.commit(win()); b1.commit(lose());}
