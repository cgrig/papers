...
Over channels, one can send \emph{messages},
  which include integers, channels, and labels.
...
\begin{align}
  &s! \langle {\tt true} \rangle; P_1 \mid s? (x); P_2
\\
...
\begin{align}
  s \triangleleft 2; P_0
  \mid
  s \triangleright \{ 1: P_1,\; 2:P_2,\; 3:P_3 \}
\end{align}
will continue with processes $P_0$~and~$P_2$ running in parallel.
In other words,
  the first process sends the value~$2$ and then continues executing~$P_0$,
  while the second process receives value~$2$ and branches according to it.

It is also possible to define a set of named procedures, and call them.
For example,
\begin{align}
\begin{aligned}
  &{\rm def}\;\{{\it foo}(y,t)=(t\triangleleft 2; P'_0)\}\;{\rm in} \\
  &\quad {\it foo}(x,s)
    \mid
    s \triangleright \{ 1: P_1,\; 2:P_2,\; 3:P_3 \}
\end{aligned}
...
is essentially equivalent to the previous example,
  provided that we obtain $P_0$ from $P'_0$ after doing argument substitutions;
  that is, $P'_0[x/y,s/t]=P_0$.
...
We can define the answer to a query~$q$ on a belief base~$T$
  as a set $\llbracket q \rrbracket_T$ of matches:
\begin{align}
  \llbracket q \rrbracket_T
  =
...
\begin{align}
  \llbracket q_1 \lor q_2 \rrbracket_T
    &= \llbracket q_1 \rrbracket_T \cup \llbracket q_2 \rrbracket_T
\\
  \llbracket q_1 \land q_2 \rrbracket_T
    &= \left\{\,\sigma_1\oplus\sigma_2
    \bigg\vert
    \on{
      (\sigma_1,\sigma_2)\in
      \llbracket q_1 \rrbracket_T \times \llbracket q_2 \rrbracket_T}%
      {\text{and $\sigma_1\bot\sigma_2$}}
%
%    \text{$(\sigma_1,\sigma_2)\in
%      \llbracket q_1 \rrbracket_T \times \llbracket q_2 \rrbracket_T$}
\,\right\}
\end{align}
...
\begin{remark}
$T \models q$ has the same meaning as: the set $\llbracket q \rrbracket_T$ of
