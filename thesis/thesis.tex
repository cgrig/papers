...
presents the interrelated types, roles and sessions. First the types pair
and expression are defined. The type pair <x> is defined such that a value
of pair of type x is constructed by pairing two values of type x. The type
expression <value> is defined using discriminated union meaning that a
value of expression <value> can be either one of the following: a value of
type <value>, an addition of two expression <value>, a subtraction of two
expression <value>, a multiplication of two expression <value>, or a
division of two expression <value>. There are two roles defined the
Calculator and the BinaryCalc. The Calculator is responsible with
evaluating expressions to a value of type <value>, while the BinaryCalc is
responsible with executing binary operations on values of type <value>. The
session EvaluateExpression <value> articulates the interaction between
three types of agents that work together in order to evaluate
expressions of type <value>. These agent types are: any, Calculator
<value>, and BinaryCalc <value>. The interaction pattern is described in a
recursive manner. The interaction is initiated by an agent of any type that
asks the Calculator agent to evaluate a expression <value>. The
EvaluateExpression session is run recursively by the Calculator agent until
a pair of <value> can be passed over to the BinaryCalc or an expression is
evaluated to a value.

Figure~\ref{fig:bcalc-rules} presents the implementation of agent Binary
Calculator and figure~\ref{fig:calc-rules} the expression Calculator
implementation, using only rules.
