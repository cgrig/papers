...
repeat$ construct only needs to be satisfied for soem fixed number of
repetitions, not forever. To model this in the operatinal semantics, we
introduce an angelic transition.
\begin{align}
(A_1,C_1,S_1)\ato(A_2,C_2,S_2) \label{eq:opsem-atran} \\
(A_1,C_1,S_1)\to(A_2,C_2,S_2) \label{eq:opsem-dtran}
\end{align}
The transition system now has angelic transitions~\eqref{eq:opsem-atran} in
addition to the demonic transitions~\eqref{eq:opsem-dtran}.
