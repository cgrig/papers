...
AF-Raf syntax is defined using the Extended Backus-Naur Form notation.

Grammar for top level structures:
\begin{verbatim}
script: directive* EOF;
directive: include | java_binding | plan | rule;

include: 'include' a=qualified_id
  {includes.add(stringOfQualifiedId($a.v,"/")+".raf");};

java_binding: action | sensor;

plan: 'plan' a=function '{' b=contract c=statement '}'
  { // TODO Check that there are no two plans or actions with the same name
    agent.plans.put($a.v.functor(), Plan.make($a.v,$b.v,$c.v)); };

rule: a='rule' b=formula '{' c=statement '}'
  {agent.rules.add(Rule.make(getSourceName()+":"+$a.getLine(),$b.v,$c.v));};

action: 'action' a=function '=' b=qualified_id ';'
  {actions.add(ActionData.make($a.v,stringOfQualifiedId($b.v,".")));};

sensor: 'sensor' a=qualified_id ';'
  {sensors.add(stringOfQualifiedId($a.v,"."));};

contract returns [ImmutableList<Contract> v]
scope {ImmutableList.Builder<Contract> builder;}:
  {$contract::builder=ImmutableList.builder();} 
  (a=contract_type b=formula ';' {$contract::builder.add(Contract.make($a.v,$b.v));})*
  {$v=$contract::builder.build();};
\end{verbatim}

Grammar for formulas and terms:
\begin{verbatim}

formula returns [IFormula v]:
  a=formula_a {$v=$a.v;}
  ('=>' b=formula_a {$v=new IMPLIES($a.v,$b.v);})?;

formula_a returns [IFormula v]:
  a=atom_formula {$v=$a.v;} 
  (b=('&' | '|') c=atom_formula
    { if ($b.text.equals("&")) $v=new AND(Lists.newArrayList($v,$c.v));
      else if ($b.text.equals("|")) $v=new OR(Lists.newArrayList($v,$c.v));
      else assert false : "internal error in parser";}
  )*;

atom_formula returns [IFormula v]:
    (UID | term pred_op) => a=predicate {$v=$a.v;}
  | '!' b=atom_formula {$v=new NOT($b.v);}
  | '(' c=formula ')' {$v=$c.v;};

predicate returns [IFormula v]:
    a=term b=pred_op c=term {$v=makePredicate($a.v,$b.v,$c.v);} 
  | d=UID '(' e=term_list? ')' {$v=new Predicate($d.text,$e.v);};

term returns [ITerm v]:
    a=term_a {$v=$a.v;}  
    (b=add_op c=term_a {$v=makeAddition($v,$b.text,$c.v);})*;

term_a returns [ITerm v]:
    a=atom_term {$v=$a.v;} 
    (b=mul_op c=atom_term {$v=makeMultiplication($v,$b.text,$c.v);})*;

atom_term returns [ITerm v]:
    '(' a=term ')' {$v=$a.v;}
  | b=literal {$v=$b.v;}
  | c=LID {$v=new Variable($c.text);}
  | d=function {$v=$d.v;};
 
function returns [Function v]: a=LID '(' b=term_list? ')'
    {$v=new Function($a.text,$b.v==null?new ArrayList<ITerm>():$b.v);};

term_list returns [List<ITerm> v]:
  a=term {$v=Lists.newArrayList($a.v);}
  (',' b=term {$v.add($b.v);})*;

pred_op returns [String v]: (a=eq_op | a=cmp_op) {$v=$a.v;};
eq_op returns [String v]: (a='==' | a='!=') {$v=$a.text;};
cmp_op returns [String v]: (a='<' | a='>') {$v=$a.text;}; // TODO: | '>=' | '<=';
add_op returns [String v]: (a='+' | a='-') {$v=$a.text;};
mul_op returns [String v]: (a='*' | a='/' | a='%') {$v=$a.text;};
unary_op returns [String v]: (a='!' | a='-') {$v=$a.text;}; // TODO where is '!' used in AFAPL?
...
statement returns [BlockStep v]
scope { List<IPlanStep> list; }:
  { $statement::list = Lists.newArrayList(); } 
  (a=atom_statement ';' {$statement::list.add($a.v);})*
  {$v=new BlockStep($statement::list);};

atom_statement returns [IPlanStep v]:
    ('{' a=statement '}'
  | a=assignment
  | a=while_statement
  | a=foreach
  ...
  | a=if_statement)
      {$v=$a.v;}
// TODO: not in examples: query, wait, try_recover, when; perhaps add later
;

assignment returns [IPlanStep v]: a=LID '=' (b=function | b=list_comprehension)
  {$v=new AssignmentStep(new Variable($a.text),$b.v,false);};

while_statement returns [IPlanStep v]: 'while' a=formula '{' b=statement '}'
  {$v=new WhileStep($a.v,$b.v);};

foreach returns [IPlanStep v]: 'foreach' a=formula '{' b=statement '}'
  {$v=new ForEachStep($a.v,$b.v);};

update_belief returns [IPlanStep v]: (a='+' | a='-') b=predicate
  {$v=new UpdateStep($a.text,$b.v);};

call returns [IPlanStep v]: a=function
  {$v=new CallStep($a.v);};

if_statement returns [IPlanStep v]: 
  'if' a=formula '{' b=statement '}' ('else' '{' c=statement '}')?
  {$v=new IfStep($a.v,$b.v,$c.v);};

list_comprehension returns [ITerm v]: '[' term '|' formula ']';
...
qualified_id returns [QualifiedId v]:
  (t=id {$v = QualifiedId.make($v, $t.text); } '.')* t=id { $v = QualifiedId.make($v, $t.text); };

contract_type returns [Contract.ContractType v]:
    (t='pre' | t='post' | t='fail')
      { $v = Contract.ContractType.valueOf($t.text.toUpperCase()); };

literal returns [ITerm v]:
    t=STRING { $v = new StringTerm(unescapeStringLiteral($t.text)); } 
  | t=INT { $v = new IntegerTerm(Integer.parseInt($t.text)); };
