...
model developed by Omicini~\cite{} and from the Jason programming language,
which is based on the Belief, Desire, Intention model. SimpAL is conceived
as an extension to the Java object oriented programming language with a
separate agent abstraction layer. 
...
multiagent systems~\cite{}. They were used for automatic generation of
...
conversation is correct conform to the global session type. 

In order to do this the developer needs to provide definitions of global
session types using Prolog rules. Global types are viewed as states, hence
the need to specify possible transitions to different states upon message
sending. Also, they need to specify the message content types. 
