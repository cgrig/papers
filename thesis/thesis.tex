...
\begin{example}
The {\tt stdio.raf} includes the following action declaration, to help
debugging.
\begin{verbatim}
  action println(s) =
    com.agentfactory.raf.RafStandardAction$Println;
...
Each {\it Rule\/} has a query\slash condition and body, which is usually a
composed statement; optionally, just for aiding debugging, it may also have
a name. The formula is obtained by connecting several predicates with
boolean operators; that is, there are no explicit quantifiers. Both
interfaces {\it IFormula\/} and {\it IPlanStep\/} are reused from the
