...
Let's look first at the binary calculator. There are four actions defined,
for each basic binary operation: intadd(x, y), intsub(x, y), intmul(x, y),
intdiv(x, y). Also, each binary operation has associated a rule saying that upon
receiving a message asking to perform that operation on a pair of integers,
the agent should first delete the message, compute the result, and send the
