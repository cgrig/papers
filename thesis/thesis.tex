...
\bigskip
Head of School: Professor P\'{a}draig Cunningham\\
...
special people around me. I would like to express my warm gratitude towards
all of them, even though the space doesn't allow me to thank each of them
individually.

Most of all, I would like to thank my supervisor, Dr Rem W. Collier, for his
meaningful guidance and continuous support. He's a role model for his
determination and hard work, but also for his big heart.

I would also like to thank all my wonderful lecturers in UCD, with special
thanks to Prof. Simon Dobson and Mr. Henry McLoughlin for giving me the
best learning experiences. In addition, I would like to thank Prof. Ita
Richardson, from University of Limerick, for offering me the best female
role model to follow.

Furthermore, I would like to thank Mikol\'{a}\v{s} Janota, Juan Ye, Oana
\c{T}ifrea, and all my friends in Dublin and Oxford for their ongoing
encouragements. Special thanks to Anca Jurcu\c{t} for her help with my
final submission, and to Florence Coetzer for proofreading the final
version of the thesis.

Many thanks to Patricia Gheoghegan, Ann O'Hanlon and Prof. Joe Carthy for their
support with the administrative side of the PhD.

I would like to thank the Irish Research Council, not only for funding this
research, but also for giving me the opportunity to attend conferences and
summer schools.

Moreover, I would like to thank my family, friends and teachers in
Romania. I am particularly grateful to my teacher, Adina Neagu, for
empowering me to work with enthusiasm, to Germina Stuparu for teaching me
how to think mathematically, and to Octavian Gavril\u{a} for his awesome
Computer Science lessons. I am thankful to all my friends and lecturers in
University of Bucharest for giving me a different perspective on life and
study. 

I am greatly indebted to Cami, Cristina, and Jane for their kindness and
generosity to look after my family while I was finalising my work.

Finally, I would like to thank my husband, Radu Grigore, for
constantly pushing me outside my comfort zone; my parents, Sabina and
Niculae Dan, for nurturing my curiosity; my siblings, Ana, Emy, and Cezar for
learning together human interaction; and my children, Mircea and Maia, for
bearing with me to finish my work. This thesis is dedicated to you.
...
\section{Motivation and Objectives}\label{sec:objectives}
...
%All of them were explored subsequently. For example
...
For example,
  popular languages like Scala~\cite{DBLP:conf/ecoop/ScalasY16}
  and Haskell~\cite{orchard2017sessions}
...
The next chapter % \autoref{ch:langdef}
...
\begin{align}
  A
  \xrightarrow{f_1(f_2(f_3(f_4(\text{``ten''}))))}
  B
  \xrightarrow{f_3(f_4(\text{``ten''}))}
  C
\end{align}
Agent~$A$ sends message $f_1(f_2(f_3(f_4(\text{``ten''}))))$ to agent~$B$.
Agent~$b$ stores a subterm,
  say $f_2(f_3(f_4(\text{``ten''})))$,
  in its belief base.
Later, when some other condition is satisfied,
  agent~$B$ extracts, from the term in its belief base,
  a subterm~$f_3(f_4(\text{``ten''}))$ which it sends to agent~$C$.
Agent~$C$ extracts a subterm~$f_4(\text{``ten''})$ and stores it in its belief base.
Later, when some other condition is satisfied, agent~$C$ extracts a
subterm~``ten''. Agent~$C$ expects ``ten'' to be an integer
literal, but instead it is a string literal. Agent~$C$ notices the
...
The concrete syntax of AF-Raf is specified by an ${\rm LL}(k)$ grammar%
  ~\cite{rosenkrantz1970}, which I call
the \emph{concrete grammar}. From the concrete grammar,
  ANTLR~\cite{DBLP:conf/pldi/ParrF11} generates a
...
actions create a tree-like data structure,
  which I call the \emph{abstract syntax tree} (AST).
The AST is obtained by instantiating certain Java classes.
The set of these Java classes are what I later call the \emph{abstract
grammar}.
...
\end{verbatim}
In BNF, one would not be allowed to use the $*$,
  and would have to write
\begin{verbatim}
  string_list ::= string string_list_tail;
  string_list_tail ::= ;
  string_list_tail ::= ',' string string_list_tail;
\end{verbatim}
or, alternatively,
\begin{verbatim}
  string_list ::= string;
...
these details about how to define {\tt string\_list} are nonessential and
ANTLR takes care of them automatically. Thus, I have decided to employ the
notation that allows the use of Kleene star~$*$.

The semantic action for the {\tt string\_list} rule would say that one has to
...
Briefly, sum types are specified as
\begin{verbatim}
  Sum <: Part1type, Part2type;
\end{verbatim}
and product types are specified as
\begin{verbatim}
  Product = Part1type part1name, Part2type part2name;
\end{verbatim}
The full Java code that is generated automatically
  from such sum and product types appears in \autoref{app:ast}.
...
Type =
  String name,
  ImmutableList<TypeBranch> actuals;
...
Why is our calculator and agent?
To answer this question,
  imagine for a moment that we implemented our calculator in Java.
Then, a legitimate question would be why is the calculator a class.
But few ask this question because the answer is more or less obvious:
  in Java, (almost) \emph{everything} is a class.
To some extent, the answer is the same in our case:
  in AF-Raf, (almost) everything is an agent.
In both cases, however, the answer --- although technically correct ---
  is not illuminating.
In both cases,
  we can find out more by probing with deeper questions.
Let us pose those question in AF-Raf's case.

Suppose that we implement a calculator in AF-Raf.
Since the fundamental entity in AF-Raf is an agent,
  we implement the calculator as an agent.
Here are the questions:
(a)~Is it natural or cumbersome to implement the calculator as an agent?
(b)~How does this implementation compare
  to the natural implementation in other languages?
  For example, how does it compare to an implementation in Java, as a class?

Let us try to address these questions.
The code in \autoref{fig:calc-op} is simple enough to read,
  but still less elegant than what one would write in Java.
Evidence that AF-Raf code is easy to read is given in \autoref{ch:results}.
As for the comparison with Java,
  observe that the slightly more complicated code of AF-Raf
  comes with a benefit:
The implementation is intrinsically distributed.
One can easily deploy several expression calculators
  that share a small number of binary calculators.
Also,
  which agent runs on which real machine,
  questions of load balancing,
  handling of network traffic, and so on
  could be handled by the underlying implementation.
Of course,
  one would need to improve AgentFactory with these features,
  but all the necessary conceptual features are there:
The programmer was guided towards writing code
  that is amenable to automated distribution.

\smallskip

Let us step back and recall which concepts guided us, intuitively,
  while designing AF-Raf, with roles and sessions.

AF-Raf sessions were inspired by global session types,
  while AF-Raf roles were inspired by local session types~%
  \cite{dblp:conf/popl/hondayc08}.
AF-Raf roles also take inspiration from Haskell type classes~%
  \cite{DBLP:conf/popl/WadlerB89},
  in particular in their use of polymorphism.
Finally, AF-Raf's basic type system takes inspiration
  from algebraic data types, which are pervasive in functional languages~%
  \cite{DBLP:journals/acta/GuttagH78}.
...
I believe that the research results confirmed our working hypotheses,
  which were formulated in \autoref{sec:objectives}:
...
\item \textit{Applying concepts from mainstream programming, like the ones
  ...
  led to a better design of an agent-oriented programming language,
  with increased efficiency and functionality.}
Our first objective was achieved by incorporating
the concept of type classes from functional programming 
  (\autoref{sec:roles}). The second objective was achieved
    by incorporating sessions (\autoref{sec:sessions}) into AF-Raf.
The third objective was achieved by designing a
  type system and a type checker that include algebraic data types
  (\autoref{sec:types}).
...
public final class RafAst {
  public static interface Ast {}
    ...
    public static final class ChoiceSessionStep implements SessionStep {
      
      private final ImmutableList<SessionInvokation> alternativeSessions;
      private ChoiceSessionStep(ImmutableList<SessionInvokation> alternativeSessions) {
        this.alternativeSessions = alternativeSessions;
      }
      public static ChoiceSessionStep make(ImmutableList<SessionInvokation> alternativeSessions) {
        return new ChoiceSessionStep(alternativeSessions);
      }
      public ImmutableList<SessionInvokation> alternativeSessions() {return alternativeSessions;}
    }
  
    public static final class Contract implements Ast {
      public static enum ContractType {PRE,FAIL,POST}
      private final ContractType type;
      private final IFormula condition;
      private Contract(ContractType type,IFormula condition) {
        this.type = type;this.condition = condition;
      }
      public static Contract make(ContractType type,IFormula condition) {
        return new Contract(type,condition);
      }
      public ContractType type() {return type;}
      public IFormula condition() {return condition;}
    }
  
    public static final class MaybeSessionStep implements NestedSessionStep {
      
      private final SessionInvokation session;
      private MaybeSessionStep(SessionInvokation session) {
        this.session = session;
      }
      public static MaybeSessionStep make(SessionInvokation session) {
        return new MaybeSessionStep(session);
      }
      public SessionInvokation session() {return session;}
    }
  
    public static final class NamedBindings implements Ast {
      
      private final String name;
      private final Bindings bindings;
      private NamedBindings(String name,Bindings bindings) {
        this.name = name;this.bindings = bindings;
      }
      public static NamedBindings make(String name,Bindings bindings) {
        return new NamedBindings(name,bindings);
      }
      public String name() {return name;}
      public Bindings bindings() {return bindings;}
    }
  
    public static final class Operation implements Ast {
      
      private final String name;
      private final Type input;
      private final Type output;
      private Operation(String name,Type input,Type output) {
        this.name = name;this.input = input;this.output = output;
      }
      public static Operation make(String name,Type input,Type output) {
...
private boolean hasTypeBranch(ITerm term, TypeBranch branch) {
    if (branch instanceof Type) {
        ...
        if (type.actuals() == null) {
            ...
            ImmutableList<String> formals = typeDeclaration.formals();
            ImmutableList<TypeBranch> actuals = type.actuals();
            if (formals.size() != actuals.size()) return false;
            UnmodifiableIterator<String> f = formals.iterator();
            UnmodifiableIterator<TypeBranch> a = actuals.iterator();
            Map<String, TypeBranch> typeSubstitution = Maps.newHashMap();
            while (f.hasNext()) {
                typeSubstitution.put(f.next(), a.next());
            }
            for (TypeBranch newBranch : typeDeclaration.branches()) {
                TypeBranch groundBranch = applyTypeSubstitution(newBranch, typeSubstitution);
                if (groundBranch == null) continue;
                if (hasTypeBranch(term, groundBranch)) return true;
            }
        }
    } else if (branch instanceof ValueConstructor) {
        ValueConstructor simpleType = (ValueConstructor) branch;
        if (term instanceof Function) {
            Function f = (Function) term;
            if (!f.functor().equals(simpleType.name())) return false;
            if (f.terms().size() != simpleType.arguments().size()) return false;
            Iterator<ITerm> subTerm = f.terms().iterator();
            UnmodifiableIterator<TypeBranch> subBranch = simpleType.arguments().iterator();
            while (subTerm.hasNext()) {
                if (!hasTypeBranch(subTerm.next(), subBranch.next())) {
                    return false;
                }
            }
            return true;
        } else {
            System.err.printf("W: untyped built-in constant term (%s)\n", term.getClass().getName());
            return true;
        }
    } else {
        Preconditions.checkArgument(false, "unsupported type branch");
    }
    return false;
}

// Returns only ground type branches.
// Instead of non-ground type branches, it returns {@code null}.
private TypeBranch applyTypeSubstitution(TypeBranch t, Map<String, TypeBranch> s) {
    if (t instanceof Type) {
        Type namedType = (Type) t;
        if (namedType.actuals() == null) {
            return s.get(namedType.name());
        } else {
            // Note that no lookup is done: All replacement is in-place.
            ImmutableList.Builder<TypeBranch> newActuals = new ImmutableList.Builder<>();
            for (TypeBranch a : namedType.actuals()) {
                a = applyTypeSubstitution(a, s);
                if (a == null) return null;
                newActuals.add(a);
            }
            return Type.make(namedType.name(), newActuals.build());
        }
    } else if (t instanceof ValueConstructor) {
        ValueConstructor simpleType = (ValueConstructor) t;
        ImmutableList.Builder<TypeBranch> newActuals = new ImmutableList.Builder<>();
        for (TypeBranch a : simpleType.arguments()) {
            a = applyTypeSubstitution(a, s);
            if (a == null) return null;
            newActuals.add(a);
        }
        return ValueConstructor.make(simpleType.name(), newActuals.build());
    } else {
        Preconditions.checkArgument(false, "unsupported type branch");
        return null;
    }
