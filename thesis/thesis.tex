...
context of Agent Oriented Programming.  \rg{Maybe promise only that
you show how roles, a concept from sociology, maps to a programming
language construct.}
...
will not have ant type errors when executed. In the weak sense only
...
elements required and provided by the module.

 In Haskell modules are used to controlling name-spaces and creating
 abstract data types. A Haskell program consists of a collection of
 modules, where the main module loads up the other modules. Loading a
 module can be seen as a big declaration. A module contains functions,
 types and type classes that are related and serve a common purpose.
...
The Calculator example demonstrates the power of operations, as well as
being a good opportunity to understand how roles are implemented, because
the roles involved are simple and clear. Figure~\ref{fig:calc-roles}
presents the interrelated types, roles and sessions associated with the
Calculator example. First the types \textit{pair} and \textit{expression}
are defined. The type \textit{pair} <x> is defined such that a value of
\textit{pair} of type x is constructed by pairing two values of type x. The
type \textit{expression} <value> is defined using discriminated union
meaning that a value of \textit{expression} <value> can be either one of
the following: a value of type <value>, an addition of two
\textit{expression} <value>, a subtraction of two \textit{expression}
<value>, a multiplication of two \textit{expression} <value>, or a division
of two \textit{expression} <value>. There are two roles defined: the
\textit{Calculator} and the \textit{BinaryCalc}. The \textit{Calculator} is
responsible with evaluating expressions to a value of type <value>, while
the \textit{BinaryCalc} is responsible with executing binary operations on
values of type <value>. The session \textit{EvaluateExpression} <value>
articulates the interaction between three types of agents that work
together in order to evaluate expressions of type <value>. These agent
types are: any, \textit{Calculator} <value>, and \textit{BinaryCalc}
<value>. The interaction pattern is described in a recursive manner. The
interaction is initiated by an agent of any type that asks the
\textit{Calculator} agent to evaluate an \textit{expression} <value>. The
\textit{EvaluateExpression} session is run recursively by the
\textit{Calculator} agent until a pair of <value> can be passed over to the
\textit{BinaryCalc} or an expression is evaluated to a value.

Figure~\ref{fig:bcalc-rules} presents an implementation of the agent binary
calculator, BinaryCalc. Figure~\ref{fig:calc-rules} is a partial
implementation of the expression calculator agent, Calculator, more exactly
implements the evaluation of the addition expressions. Both of these
implementations are using only rules.  Figure~\ref{fig:calc-op} presents a
much simpler and shorter implementation of agents BinaryCalc and
...
\end{verbatim}
\caption{Calculator agent implementation}
...
programming language as concisely as possible. AF-Raf's abstract grammar is
presented in~\autoref{fig:Abstract-Grammar}. It describes the abstract
syntax trees generated by the parser and used by the type checker and the
interpreter.
...
a design name, a list of arguments, a list of roles, and a list or rules.
Each argument ({\it AgentRole\/} in the abstract syntax) consists of a name
and a role. Suppose the design arguments are declared to be
$n_1\;r_1$,~$n_2\;r_2$, $\ldots$ Then, in order to instantiate the design,
one will need to provide the addresses of agents that play the roles
$r_1$,~$r_2$, \dots, and the rules of the design may refer to these
addresses using the names $n_1$,~$n_2$, $\ldots$ (To a first approximation,
the design name is analogous to Java class names, and the arguments are
analogous to the arguments of a Java constructor.)

References to agent types (roles) and to value types are represented in the
abstract grammar by {\it Type}, because of their similarity. Both roles and
types have names, and both may be parametrized by types. Thus, a reference
to a role/type consists of the name of the role/type together with the
types of the parameters.

On the other hand, declarations of roles are quite different from
declarations of types.

\todo{Declarations of roles.}
...
variables of the type being defined.
...
\rg{This description should be a little more high-level. Perhaps include
examples here and there?}

The syntax of AF-Raf is described using the Extended Backus-Naur Form notation.
The grammar for top level structures is as follows:
\rg{If you keep the actual grammar here, you must strip it of all
non-essential parts, if you want any chance of it being readable. For
example, no `a=' and `b='.}
...
more times. 
\rg{Here just say that terms use operators bla-bla, and that term\_a,
term\_b and so on are just a standard trick to encode operator precedence
in LL grammars. You can give a table with the operator precedences, and
perhaps add a citation for LL grammar.}
