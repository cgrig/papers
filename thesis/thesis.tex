...
communication and interaction to the studied processes.  Within Distributed
Artificial Intelligence appeared the actor models as a way of performing
concurrent computation based on message passing. Based on this, emerged the
idea of open systems, which are systems in continuous interaction with an
external environment. Next step was to build a society of interacting
autonomous agents.
...
The core components of the mental state are beliefs, commitments and
capabilities. Shoham argues that more complex mental notions such as
desires, intentions, goals or plans can be included, but those three are
...
Agent0 is a prototype programming language, and it includes only constructs
related to the agent level of a multiagent system. Moreover, Shoham only
mentioned that a society of agents would need further organisation in order
to work properly. He suggested that the roles and norms could be
appropriate options to consider, but there was no concrete implementation
for any social level constructs.

A rich profusion of agent oriented programming languages followed Agent0
focusing on the mental notions and implying a societal view of computation.
Many of them are heavily influenced by the Belief, Desire, Intention agent
architecture as well as by the reactive planning, which represents a group
of techniques for action selection by autonomous agents.  Notable agent
oriented programming languages, from a historical point of view, are
AgentSpeak, MetateM, 3APL, Jason, Jadex, and GOAL.

TODO: AOSE

A diversity of language constructs are employed to model the abstract
concepts used in Agent Oriented Software Engineering. These abstract
concepts include beliefs, goals, intentions, plans, groups, norms,
...
language constructs are first-class constructs. The
individual(agent) level had most of the attention so far, with environment
coming after, and social level lagging behind. Only recently the social
level of multiagent systems has started to receive our attention at the
language level.

TODO: Talk about the history of Agent Oriented Programming starting with
Yoav Shoham's Agent0. Give examples of programming languages discussing
their strengths and weaknesses. Talk about metaphors and paradigms (like
BDI).
% >>>
\section{Role Theory} % <<<
...
There are also three types of rules: generate plans to achieve goals,
process internal events and received messages, and to handle and repair
failed plans.
...
provide rules for processing information received from the environment.
...
distinguishes different types of goals are coexist primitive, achievement
or achieved goals.  \cite{} GOAL Agents Instantiate Intention Logic

TODO: Draw a table with all the programming languages presented and their main
features.
\\
