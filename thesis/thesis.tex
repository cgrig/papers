...
The relevant parts of the research literature is surveyed next
...
design of AF-Raf. We've also looked at all the important concepts from
functional languages, type theory, and  multiparty session types, on which
we base our analogies. A brief description of Agent Factory is presented.

The next part (\autoref{part:AF-Raf}) includes two chapters that present the language AF-Raf. The first of these
...
Last part (\autoref{part:eval})contains a chapter with two case studies
(\autoref{ch:casestudy}), along with a survey analysis
(\autoref{ch:survey}), and a summary of the results and evaluation
(\autoref{ch:results}).
...
to group a number of roles into a team.  Teams combine the concepts of
...
that they contain a set of classes~\cite{DBLP:journals/ao/Herrmann07}.
...
Brute facts are the state of the multiagent organisation, which is
