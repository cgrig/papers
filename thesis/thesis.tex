...
AOP can be viewed as a specialisation of OOP. OOP sees the computational system
as made up of units that are able to communicate using messages. AOP makes this
framework more specific by restricting the state of the unit to consist only of
...
communication framework. The Analysis phase define system goals from a set of
requirements and then define the roles necessary to meet the goals. A role
consists of a number of tasks, where a task corresponds to a goal. Each task
has its own thread of control, but may communicate with each other. The Design
phase assigns roles to agent classes, identify and constructs conversations
(coordination protocols between two agents), define the internal agent
architecture and the final system structure. 
...
A different approach to support the implementation of organisational concepts would be to develop custom programming languages, in the context of Agent Oriented Programming. The majority of AOP
...
Agent Oriented Programming, the relatively new programming paradigm, adopts a
...
languages. The objective is to develop a novel AOP language that employs roles, based on the existing Agent Factory framework.
...
% >>>
\chapter{Research Findings}\label{ch:method} % <<< 
...
without making it to difficult to use.
...
The main particularity of AF-Raf, that makes it unique among all other
agent oriented programming languages is that, even though it is built upon
the agent oriented conceptual framework, it also draws inspiration from
...
oriented programming language to introduce types and type-checking. From a
practical point of view, our effort tries to improve upon the following
scenario.
...
agent~$a$ can process. In order to send a message to agent~$a$, other agent
need the agent identifier of agent~$a$. Agent identifiers typically consist of
