...
\lstdefinestyle{hs}{language=haskell,deletekeywords={elem,all,Eq,Num}}
\lstdefinestyle{me}{language=haskell,keywords={role,agent,plays,PG-rules,true,this,return}}
...
to name both the agent and the role it plays.
\begin{lstlisting}[style=me]
agent foo plays Calculator Int(n)
  PG-rules:
    eval(Ct(x)) <- return(x)
    eval(Times(x, y)) <-
      return(n.multiply(this.eval(x), this.eval(y)))
    eval(Plus(x, y)) <-
      return(n.add(this.eval(x), this.eval(y)))
\end{lstlisting}
This code assumes some modifications of 2APL's syntax and, more
importantly, some additions to its semantics. The simplest syntax
modification is that the belief query \textbf{true} is omitted, so the
rules above have the shape
$\langle\mathit{GoalQuery\rangle\gets\langle\mathit{Plan}\rangle}$.  The
semantic modifications are subtle, but aim at making the rules easy to
write and read. When a message with the content
$\mathit{eval}(\mathit{call}(n)),x$ is received the goal $\mathit{eval}(x)$
is added to the goal list, the address of the sender is saved, and the
address $n$ is saved. \todo{continue}
